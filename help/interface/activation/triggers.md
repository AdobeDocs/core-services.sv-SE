...
4. Konfigurera utlösaren genom att fylla i följande fält och dra mått och dimensionsobjekt till regelns behållare:
   ...
   | [!UICONTROL Description] | Beskrivningen av den här utlösaren, hur du kommer att använda den och så vidare. |
   | [!UICONTROL Report Suite] | Analytics [rapportsviten](https://experienceleague.adobe.com/docs/analytics/implementation/analytics-basics/ref-reports-report-suites.html?lang=en) som används för den här utlösaren. Den här inställningen identifierar de rapportdata som ska användas. |
